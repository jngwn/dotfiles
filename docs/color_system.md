...
The token tables and usage rules in this document are the source of truth for
shared semantic palette decisions. Tool configuration files are manually
maintained consumers. They do not need to expose every token, and an intentional
tool-specific mapping may differ when the usage rules below explain why.

The maintained color system is an independent design and compatibility layer.
Removing a language, tool, plugin, extension, UI component, or workflow must not
modify its color entries, their comments, or their mapping metadata, and
must not add lifecycle annotations solely because a consumer was removed.
Behavioral activation references may change with their owning behavior. Mapping
definitions are changed or pruned only as an explicit color-system decision.

The `Maintained mappings` columns identify reviewed consumers or compatibility
targets. They do not assert that every listed component is installed or active.

Primary consumers:

- `config/foot/foot.ini`
- `config/nvim/colors/paper-custom.vim`
- `home/.vscode/extensions/local.paper-custom-color-theme/themes/paper-custom-color-theme.json`
...
There is no generated palette artifact. Change the semantic token here and the
affected consumers in the same reviewed change. Record an intentional override
in the relevant usage rule instead of silently allowing the shared values to
drift. Add a mechanical drift gate only if repeated mismatches demonstrate that
manual synchronization is no longer reliable.

`paper-custom.vim` is the maintained Neovim colorscheme and contains the
terminal ANSI palette used by Neovim. If it cannot load, `init.lua` reports the
failure and falls back to Neovim's `default` colorscheme.

The VS Code `Paper Custom` theme is a local declarative extension deployed under
`~/.vscode/extensions/`. It has no executable extension host code, network
activity, update checker, or persistent state. Its unversioned deployment
directory is intentional: the dotfile deployment replaces its files in place
without leaving obsolete theme versions beside the maintained copy.

VS Code intentionally maps its title bar, activity bar, and ordinary status bar
to `paper.gutter` rather than the general chrome tokens. This keeps the outer
frame visually continuous with the editor gutter; borders, focus indicators,
badges, debugging, warnings, and errors retain their semantic colors.

### Third-Party Provenance

| Artifact | Upstream | Imported revision | Local status | License and attribution | Deployment path |
|---|---|---|---|---|---|
| `config/nvim/colors/paper-custom.vim` | `https://github.com/yorickpeterse/vim-paper.vim` | Unknown; the original import revision was not recorded | Locally maintained derivative | Upstream author, website, and `MPL 2.0` notice remain in the file header | `~/.config/nvim/colors/paper-custom.vim` |

Do not guess the missing upstream revision. Before the next upstream refresh,
identify the replacement revision in the reviewed change and update this table
with the resulting modification status and any required notice handling.

## Core Palette

| Token | Hex | Role | Maintained mappings |
|---|---:|---|---|
| `paper.canvas` | `#f2eede` | Main background | Terminal backgrounds, Codex background, Nvim `Normal`, Starship directory |
...
| `paper.chromeActive` | `#b8ad94` | Active chrome | Nvim statusline, tmux current window |
...
| `ink.secondary` | `#303030` | Secondary text | Nvim line numbers and non-text, Starship detached commit |
...
| `accent.go` | `#006f8a` | Darkened Gopher Blue for readable Go identity | Yazi current directory, table columns, key chords |
...
| `accent.comment` | `#2f5f8f` | Comments and CLI highlights | Nvim comments, Codex comments |
...
| Token | Hex | Role | Maintained mappings |
|---|---:|---|---|
| `state.selection` | `#b7c9dc` | Standard selection and passive attention | Terminal selection, Codex selection, Nvim visual selection, popup selection, tmux copy mode and background-task bell, Sway urgency |
| `state.search` | `#ffd400` | Search and command messages | Nvim search, tmux message |
...
This keeps terminal output visually consistent with Neovim, Starship, and tmux.
Some ANSI colors are softer on `paper.canvas`; tools that emit their own
...
Translate semantic roles to the target terminal's current configuration model;
do not mechanically copy similarly named settings. When the target does not
expose an equivalent semantic slot, leave it unsupported rather than assigning
the token to a different concept or inventing a replacement color.

Theme parity covers the documented foreground, background, cursor, selection,
search, and ANSI mappings, together with settings that alter their
interpretation. Renderer-specific glyph rasterization, gamma, color management,
UI structure, and unsupported derived dim colors are outside the palette parity
contract. Applications that emit truecolor or replace the terminal palette at
runtime remain responsible for their own mappings.
...
This is a reviewed dormant mapping retained for future fuzzy-finder consumers;
the maintained shell baseline does not currently configure one.

The dormant BSD `ls` mapping is
`LSCOLORS=gxfxcxdxbxegedabagacad`. The maintained Arch shell baseline exports
`LS_COLORS` instead. Keep `LSCOLORS` only as an inactive color-system reference,
not as a supported-platform setting.
...
theme must set those tokens directly instead of relying on the terminal or
Neovim terminal ANSI colors. Use `state.diffDeleteSoft` with
...
- `#9fc5e8`: brighter selection blue. It is more immediately visible for
  ...
  `state.selection` (`#b7c9dc`) across Nvim and tmux.
