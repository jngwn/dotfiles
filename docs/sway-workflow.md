...
삭제한 앱이 실행기 목록에 남아 있으면 먼저 `~/.local/share/applications/`에서 관련 `.desktop` 파일을 찾고, 없으면 `/usr/local/share/applications/`와 `/usr/share/applications/`도 확인한다. 파일의 `Exec`, `TryExec`, `DBusActivatable` 값과 설치 출처를 확인한다. 삭제한 앱의 사용자 항목만 직접 지우고, 시스템 항목은 소유 패키지나 설치 도구를 통해 정리한다. `~/.config/autostart/`는 로그인 자동 실행 설정이므로 실행기 목록과 별개로 확인한다.
