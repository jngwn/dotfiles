...
provider가 있을 때만 OS clipboard를 사용한다. WSL의 tmux와 Neovim은 Windows
clipboard executable을 직접 호출하지 않고 terminal integration을 사용한다. Neovim은
WSL과 SSH에서 unnamed yank만 terminal로 전달하고 일반 paste는 editor register에
남긴다. Host clipboard 입력은 terminal paste가 소유한다. Application별 clipboard
선택은 각 component가 소유하며 같은 locality 경계를 지킨다. Provider 우선순위는 더
구체적인 환경이 일반 환경을 덮어쓰도록 정의한다. SSH에서는 host GUI provider를 직접
추측하지 않고 terminal integration과 tmux buffer를 fallback으로 사용한다.
...
**Why:** 일반 편집 동작, language intelligence와 project 탐색의 설정 지점을 분리하면
내장 기능을 유지하면서 각 편집 구성을 독립적으로 읽고 변경할 수 있다.

**Boundary:** `config/nvim/init.lua`가 일반 editor runtime을,
`config/nvim/plugin/packages.lua`가 외부 plugin 선언을, 그 밖의 `config/nvim/plugin/*.lua`가
독립적으로 자동 load되는 기능을, `config/nvim/after/plugin/*.lua`가 plugin별 설정을,
`config/nvim/lsp/*.lua`가 언어별 server 설정을 소유한다. `init.lua`는 저장소의 다른 Neovim
설정 파일을 `require`하거나 `source`하지 않으며, 해당 파일이 없어도 기본 편집 session이 오류
없이 시작되어야 한다. 분리된 설정의 부재는 그 설정이 제공하는 기능만 비활성화한다. 기존
fold와 의도적으로 비활성화한 rollback option을 보존하고, language intelligence는 Neovim 내장
LSP client로 구현한다. Plugin은 내장 기능으로 대체할 수 없는 경우에만 사용한다. Project 단위
source와 file search는 quickfix navigation 범위에서 editor가 담당하며 dependency 관리, build,
