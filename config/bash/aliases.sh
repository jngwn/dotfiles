...
alias alish='test -f ~/.config/bash/aliases.sh && vim ~/.config/bash/aliases.sh || echo "WARN: File does not exist."'
...
reload_config() {
  ...
  # edits require a manual setup_dotfiles.sh deployment before this can apply them.
  local failed=false
  ...
  if [[ -f "${HOME}/.config/bash/aliases.sh" ]]; then
    ...
    if ! source "${HOME}/.config/bash/aliases.sh"; then
      echo "ERROR: Failed to source the Bash aliases."
      failed=true
    fi
  else
    echo "ERROR: Bash aliases file does not exist: ${HOME}/.config/bash/aliases.sh"
    failed=true
  fi
...
_clear_shell_history() {
  ...
  local -r expected_history_file="${HOME}/.bash_history"
...
export LS_COLORS="di=01;34:ln=36:ex=32:ow=01;34:tw=01;34:*.sh=31:*.bash=31:*.rs=33:*.go=38;2;0;111;138:*.c=34:*.h=36:*.cc=34:*.cpp=34:*.java=31:*.json=36:*.toml=33:*.yaml=33:*.yml=33:*.zip=31:*.7z=31:*.tar=31:*.gz=31"
