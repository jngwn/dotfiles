...
Install a clean terminal/base Arch system, reboot, then run
`scripts/setup_arch_bootstrap.sh` to install Sway, greetd, Fcitx5 Hangul input,
user tools, and network policy. Menu names change across archinstall releases;
run `archinstall --dry-run` on the live ISO when the exact current menu or config
keys are needed.
...
- Disk encryption: enable LUKS by default for the system and user-data
  ...
  - Leave dm-crypt discard/TRIM pass-through disabled for encrypted system
    volumes. This intentionally reduces exposure of encrypted block-use patterns
    at the cost of potential SSD maintenance and sustained-write performance;
    enable it only after an explicit privacy-policy decision based on a
    demonstrated need.
...
## Arch Linux on WSL: User Setup

This README keeps a standalone account setup note for occasional Arch Linux WSL
use. No other repository configuration, script, or maintained document supports
WSL; dotfile deployment and OS bootstrap remain Arch Linux + Sway only.
...
For a distribution with a launcher executable, change the default user from
Windows PowerShell:
...
Imported distributions do not provide that launcher command. Set their default
user in `/etc/wsl.conf` instead:
...
Then restart the distribution from Windows PowerShell:
...
### Setup Logs, Backups, and Package Workspaces
...
The bootstrap also leaves package workspaces under `~/tmp/packages`:

- `yay/` contains the AUR checkout and build files. The installed `yay` command does not depend on this directory. Keep it while diagnosing or retrying installation, then inspect and remove it manually when it is no longer needed.
- `nerd_fonts_setup/` contains the downloaded archive and extracted font files. Installed fonts are copied to `~/.local/share/fonts` and do not depend on this directory. Inspect and remove the workspace manually after confirming the font installation.
