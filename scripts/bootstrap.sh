...
install_pipewire_jack() { # {{{
  echo ""
  echo "INFO: Installing pipewire-jack; confirm any JACK provider replacement prompt."
  # Keep provider removal and pipewire-jack installation in one pacman transaction.
  if ! run_as_root pacman -S --needed -- pipewire-jack; then
    echo "WARN: Failed to install pipewire-jack."
    return 1
  fi
} # }}}
...
handle_hardware_drivers() { # {{{
  if is_wsl; then
    echo "INFO: Skipping hardware driver setup in WSL."
    return 0
  fi
  ...
  # Rebuild explicitly after changing the GPU module stack; package hooks may vary.
...
setup_alsa_auto_mute() { # {{{
  if is_wsl; then
    echo "INFO: Skipping ALSA hardware setup in WSL."
    return 0
  fi
...
_install_required_package_groups() { # {{{
  ...
  for group_name in "${@}"; do
    local group_packages_output=""
    if ! group_packages_output="$(pacman -Sgq "${group_name}" 2>/dev/null)"; then
    ...
    mapfile -t group_packages < <(printf '%s\n' "${group_packages_output}" | sort -u)
...
install_base_packages() { # {{{
  ...
  _install_required_packages \
    ...
    zip unzip 7zip \
    tree yazi mat2 fontconfig ffmpeg glib2 \
    pacman-contrib || failed=true

  if is_wsl; then
    _install_required_packages xdg-user-dirs || failed=true
    [[ "${failed}" == "false" ]]
    return
  fi
...
install_desktop_foundation_packages() { # {{{
  if is_wsl; then
    echo "INFO: Skipping desktop foundation packages in WSL."
    return 0
  fi
  ...
  install_pipewire_jack || failed=true
  _install_required_packages \
    noto-fonts-cjk noto-fonts-emoji dbus \
...
install_sway_session_packages() { # {{{
  if is_wsl; then
    echo "INFO: Skipping Sway session packages in WSL."
    return 0
  fi
...
install_desktop_application_packages() { # {{{
  if is_wsl; then
    echo "INFO: Skipping desktop application packages in WSL."
    return 0
  fi
  ...
  _install_required_packages \
    ...
    firefox mpv ghostty btop thunar tumbler thunar-archive-plugin xarchiver gvfs \
...
setup_flatpak_remote() { # {{{
  if is_wsl; then
    echo "INFO: Skipping Flatpak setup in WSL."
    return 0
  fi
...
setup_local_container_runtime() { # {{{
  if is_wsl; then
    echo "INFO: Skipping guest container service setup in WSL."
    return 0
  fi
...
# Arch Linux user and system configuration {{{
...
setup_kanshi_local_profile() { # {{{
  if is_wsl; then
    echo "INFO: Skipping Kanshi output profile setup in WSL."
    return 0
  fi
...
setup_gtk_file_chooser_preferences() { # {{{
  if is_wsl; then
    echo "INFO: Skipping GTK file chooser setup in WSL."
    return 0
  fi
...
setup_gtk_bookmarks() { # {{{
  if is_wsl; then
    echo "INFO: Skipping GTK bookmark setup in WSL."
    return 0
  fi
  ...
  for bookmark_spec in "${bookmark_specs[@]}"; do
    ...
    if [[ "${xdg_name}" != "PROJECTS" ]] && command -v xdg-user-dir >/dev/null 2>&1; then
      local resolved_xdg_dir=""
      resolved_xdg_dir="$(run_as_target_user xdg-user-dir "${xdg_name}" 2>/dev/null || true)"
      if [[ -n "${resolved_xdg_dir}" && "${resolved_xdg_dir}" != "${target_user_home}" ]]; then
        bookmark_dir="${resolved_xdg_dir}"
      fi
    fi
    ...
    managed_bookmarks+=("${bookmark_uri} ${bookmark_label}")
  done

  local -r gtk_config_dir="$(dirname "${gtk_bookmarks}")"
  run_as_target_user mkdir -p "${gtk_config_dir}" || {
    echo "ERROR: Could not create the GTK config directory."
    return 1
  }
  if [[ -L "${gtk_bookmarks}" ]] ||
    [[ -e "${gtk_bookmarks}" &&
      (! -f "${gtk_bookmarks}" || ! -O "${gtk_bookmarks}" || ! -r "${gtk_bookmarks}") ]]; then
    echo "ERROR: Refusing to replace an unsafe GTK bookmark file: ${gtk_bookmarks}"
    return 1
  fi

  local existing_bookmark
  local managed_label
  local preserve_bookmark
  local -a merged_bookmarks=("${managed_bookmarks[@]}")
  if [[ -r "${gtk_bookmarks}" ]]; then
    while IFS= read -r existing_bookmark || [[ -n "${existing_bookmark}" ]]; do
      [[ -n "${existing_bookmark}" ]] || continue
      preserve_bookmark=true
      for bookmark_spec in "${bookmark_specs[@]}"; do
        managed_label="${bookmark_spec#*:}"
        if [[ "${existing_bookmark}" == *" ${managed_label}" ]]; then
          preserve_bookmark=false
          break
        fi
      done
      if [[ "${preserve_bookmark}" == "true" ]]; then
        merged_bookmarks+=("${existing_bookmark}")
      fi
    done <"${gtk_bookmarks}"
  fi

  local bookmark_temp=""
  bookmark_temp="$(run_as_target_user mktemp "${gtk_config_dir}/bookmarks.XXXXXX")" || {
    echo "ERROR: Could not create a temporary GTK bookmark file."
    return 1
  }
  if ! printf '%s\n' "${merged_bookmarks[@]}" | run_as_target_user tee "${bookmark_temp}" >/dev/null ||
    ! run_as_target_user chmod 0600 "${bookmark_temp}" ||
    ! run_as_target_user mv -f -- "${bookmark_temp}" "${gtk_bookmarks}"; then
    run_as_target_user rm -f -- "${bookmark_temp}" >/dev/null 2>&1 || true
    echo "ERROR: Could not write GTK bookmarks: ${gtk_bookmarks}"
    return 1
  fi
  echo "INFO: Applied GTK Places shortcuts and preserved custom bookmarks: ${gtk_bookmarks}"
} # }}}

validate_sway_desktop_configuration() { # {{{
  if is_wsl; then
    echo "INFO: Skipping Sway configuration validation in WSL."
    return 0
  fi
...
validate_deployed_user_units() { # {{{
  if is_wsl; then
    echo "INFO: Skipping desktop user-unit validation in WSL."
    return 0
  fi
...
setup_desktop_system_integration() { # {{{
  if is_wsl; then
    echo "INFO: Skipping desktop system integration in WSL."
    return 0
  fi
...
set_graphical_boot_target() { # {{{
  if is_wsl; then
    echo "INFO: Skipping graphical boot target setup in WSL."
    return 0
  fi
...
setup_desktop_system_services() { # {{{
  if is_wsl; then
    echo "INFO: Skipping desktop system services in WSL."
    return 0
  fi
...
setup_sway_login_session() { # {{{
  if is_wsl; then
    echo "INFO: Skipping Sway login session setup in WSL."
    return 0
  fi
...
setup_locale() { # {{{
  if locale -a 2>/dev/null | grep -Fxq "en_US.utf8"; then
    if is_wsl && grep -Fxq "LANG=en_US.UTF-8" /etc/locale.conf 2>/dev/null; then
      echo "DONE: System locale is already UTF-8."
      return 0
    fi
    if ! is_wsl && localectl status 2>/dev/null | grep -Eq '^System Locale: LANG=en_US\.UTF-8$'; then
      echo "DONE: System locale is already UTF-8."
      return 0
    fi
  fi
  ...
  if is_wsl; then
    printf 'LANG=en_US.UTF-8\n' | run_as_root tee /etc/locale.conf >/dev/null || {
      echo "WARN: Failed to configure the WSL locale."
      failed=true
    }
  else
...
setup_date_and_time() { # {{{
  if is_wsl; then
    echo "INFO: Skipping host-owned date and time setup in WSL."
    return 0
  fi
...
setup_wsl_ssh_agent() { # {{{
  if ! is_wsl; then
    echo "INFO: Skipping WSL SSH agent setup on a native system."
    return 0
  fi

  echo ""
  echo "INFO: Configuring the WSL SSH agent..."

  if [[ "${XDG_RUNTIME_DIR:-}" != /* ]]; then
    echo "ERROR: XDG_RUNTIME_DIR is unavailable; the systemd user manager is not ready."
    return 1
  fi
  if ! command -v systemctl &>/dev/null; then
    echo "ERROR: systemctl is required to configure the WSL SSH agent."
    return 1
  fi
  if ! command systemctl --user show-environment >/dev/null 2>&1; then
    echo "ERROR: The systemd user manager is unavailable. Enable systemd in /etc/wsl.conf and restart WSL."
    return 1
  fi

  local socket_load_state=""
  socket_load_state="$(command systemctl --user show ssh-agent.socket --property=LoadState --value 2>/dev/null)"
  if [[ "${socket_load_state}" != "loaded" ]]; then
    echo "ERROR: The OpenSSH user socket is unavailable: ssh-agent.socket"
    return 1
  fi

  if ! command systemctl --user enable --now ssh-agent.socket; then
    echo "ERROR: Failed to enable the WSL SSH agent socket."
    return 1
  fi
  if [[ ! -S "${XDG_RUNTIME_DIR}/ssh-agent.socket" ]]; then
    echo "ERROR: The WSL SSH agent socket was not created."
    return 1
  fi

  echo "DONE: The WSL SSH agent socket is active."
} # }}}
...
create_default_directories() { # {{{
  ...
  create_user_dir() {
    ...
    if [[ -e "${dir_path}" || -L "${dir_path}" ]]; then
      if [[ ! -d "${dir_path}" ]]; then
        echo "ERROR: Expected a user directory but found another file type: ${dir_path}"
        return 1
      fi
      echo "DONE: Preserving existing user directory attributes: ${dir_path}"
      return 0
    fi
...
install_nerd_font() { # {{{
  if is_wsl; then
    echo "INFO: Skipping guest font installation in WSL."
    return 0
  fi
  ...
  install_commit_mono_nerd_font() {
    local -r install_id="${version} ${archive_sha256}"
    local -r install_marker="${font_dir}/.dotfiles-${font_name}.version"
    local installed_id=""
    if [[ -f "${install_marker}" && -r "${install_marker}" ]]; then
      installed_id="$(<"${install_marker}")"
    fi

    if [[ "${installed_id}" == "${install_id}" ]] &&
      find "${font_dir}" -maxdepth 1 -type f -name "${font_name}-*.otf" -print -quit | grep -q "."; then
      echo "DONE: ${font_name} ${version} is already installed. Skipping..."
      return 0
    fi
    ...
    local -r temp_dir="${xdg_cache_home}/dotfiles/bootstrap/nerd-fonts/${version}"
    local -r archive_path="${temp_dir}/CommitMono.zip"
    run_as_target_user mkdir -pv "${temp_dir}" || {
      echo "WARN: Failed to create the ${font_name} cache directory."
      return 0
    }
    ...
    if ! run_as_target_user find "${temp_dir}" -type f -name "${font_name}-*.otf" -print -quit | grep -q "."; then
      echo "WARN: ${font_name} files were not found in the reviewed archive layout."
      return 0
    fi

    run_as_target_user mkdir -pv "${font_dir}" || {
      echo "WARN: Failed to create the user font directory."
      return 0
    }

    run_as_target_user find "${temp_dir}" -type f -name "${font_name}-*.otf" \
      -exec cp -f -- {} "${font_dir}/" ";" || {
    ...
    printf "%s\n" "${install_id}" | run_as_target_user tee "${install_marker}" >/dev/null || {
      echo "WARN: Failed to record the installed ${font_name} version."
      return 0
    }
...
setup_system_data_retention() { # {{{
  if is_wsl; then
    echo "INFO: Skipping system journal retention setup in WSL."
    return 0
  fi
...
setup_failed_authentication_policy() { # {{{
  if is_wsl; then
    echo "INFO: Skipping system authentication policy setup in WSL."
    return 0
  fi
...
setup_firefox_policy() { # {{{
  if is_wsl; then
    echo "INFO: Skipping guest Firefox policy setup in WSL."
    return 0
  fi
...
setup_basic_firewall() { # {{{
  if is_wsl; then
    echo "INFO: Skipping guest firewall setup in WSL."
    return 0
  fi
  ...
  setup_firewalld_firewall() {
    ...
    if ! command -v systemctl &>/dev/null; then
      echo "ERROR: systemctl is required to configure firewalld."
      return 1
    fi
    if ! command -v firewall-cmd &>/dev/null; then
      echo "ERROR: firewall-cmd is required to configure firewalld."
      return 1
    fi
    if ! run_as_root systemctl enable --now firewalld.service; then
    ...
    if [[ -n "${SSH_CONNECTION:-}" ]]; then
      ...
      if ! run_as_root firewall-cmd --permanent --add-service=ssh; then
        echo "ERROR: Failed to add an SSH allow rule in firewalld."
        return 1
      fi

      if ! run_as_root firewall-cmd --reload; then
        echo "ERROR: Failed to reload firewalld after adding the SSH allow rule."
        return 1
      fi
      ...
      if ! default_zone="$(firewall-cmd --get-default-zone)"; then
        echo "ERROR: Could not inspect the default firewalld zone; its rules were preserved."
        return 1
        ...
        if ((ssh_query_status == 0)); then
          ...
          if run_as_root firewall-cmd --permanent --zone=public --remove-service=ssh; then
            if ! run_as_root firewall-cmd --reload; then
              echo "ERROR: Failed to reload firewalld after removing the packaged SSH rule."
              return 1
            fi
          else
            echo "ERROR: Failed to remove the packaged SSH allow rule from firewalld."
            return 1
          fi
        elif ((ssh_query_status != 1)); then
          echo "ERROR: Could not inspect the public-zone SSH rule; it was preserved."
          return 1
        fi
      fi
    fi
    ...
    if command -v ufw &>/dev/null; then
      if ! run_as_root systemctl disable --now ufw.service >/dev/null 2>&1; then
        echo "ERROR: Failed to disable UFW after selecting firewalld."
        return 1
      fi
    fi
  }
  ...
  setup_ufw_firewall() {
    ...
    if ! command -v ufw &>/dev/null; then
      echo "ERROR: ufw is required to configure the selected firewall backend."
      return 1
    fi

    if [[ -n "${SSH_CONNECTION:-}" ]]; then
      echo "INFO: SSH session detected. Ensuring inbound SSH remains allowed before enabling UFW..."
      if ! run_as_root ufw allow OpenSSH && ! run_as_root ufw allow 22/tcp; then
        echo "ERROR: Failed to add an SSH allow rule before enabling UFW."
        return 1
      fi
    fi

    if ! run_as_root ufw default deny incoming; then
      echo "ERROR: Failed to set the default inbound UFW policy."
      return 1
    fi
    if ! run_as_root ufw default allow outgoing; then
      echo "ERROR: Failed to set the default outbound UFW policy."
      return 1
    fi
    if ! run_as_root ufw logging low; then
      echo "ERROR: Failed to configure UFW logging."
      return 1
    fi
    if ! echo "y" | run_as_root ufw enable; then
      echo "ERROR: Failed to enable UFW."
      return 1
    fi
    if command -v systemctl &>/dev/null; then
      if ! run_as_root systemctl enable ufw.service >/dev/null 2>&1; then
        echo "ERROR: Failed to enable the UFW service."
        return 1
      fi
    fi
  }
...
setup_networkmanager_privacy() { # {{{
  if is_wsl; then
    echo "INFO: Skipping NetworkManager privacy setup in WSL."
    return 0
  fi
...
setup_basic_network_privacy() { # {{{
  if is_wsl; then
    echo "INFO: Skipping guest network privacy setup in WSL."
    return 0
  fi
...
setup_trusted_network_profiles() { # {{{
  if is_wsl; then
    echo "INFO: Skipping trusted network profiles in WSL."
    return 0
  fi
...
show_completion_notice() { # {{{
  ...
  if is_wsl; then
    echo "DONE: WSL bootstrap complete."
    echo "INFO: If WARN lines appeared, review them before starting a new shell."
  else
...
main() { # {{{
  ...
  if ! is_supported_platform; then
    ...
    echo "   bootstrap.sh supports Arch Linux on native systems and WSL."
    exit 1
  fi
