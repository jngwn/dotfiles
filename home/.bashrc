...
_bash_history_dir="${XDG_STATE_HOME}/bash"
[[ -d "${_bash_history_dir}" ]] || mkdir -p "${_bash_history_dir}"
export HISTFILE="${_bash_history_dir}/history"
unset _bash_history_dir
...
# Keep prompts local: separate repeated prompts, then show path and input.
_set_prompt() {
  local status=$?
  local context=''
